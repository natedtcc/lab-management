...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['student_id'])) {
  ...
  if (isset($_SESSION['student_id'])) $temp_student_id = $_SESSION['student_id'];
  else error_string_gen('An error occured.');


  if (isset($_SESSION['drive_id'])) $drive_id = $_SESSION['drive_id'];
  else error_string_gen('An error occured.');
  ...
  if (isset($trimmed['description'])) {
    ...
    else error_string_gen('Server number mismatch. Hit back and try again.');


    // Verify that the server is unoccupied with a query to the
    // servers database

    $server_query = 'SELECT * FROM servers WHERE server_num='
      . $temp_server_num . ' AND available = 1';
    ...
    if ($drive_id && $server_num && $student_id && $description) {
      ...
      $server_assign_query = "INSERT INTO `server_assignments`(`server_num`, "
        . "`student_id`, `description`, `lease_time`, `expire_time`) "
        . "VALUES ($server_num,$student_id,\"$description\",NOW(), '$lease_end')";

      $server_availabiltiy_query = "UPDATE servers SET available = 0 WHERE "
      ...
      if ($result_a && $result_b) {
        ...
        /* The next conditionals are used to format the spacing in the log file, 
        specifically the server number, description and student id. */
        ...
        $update .= "$server_string\t$student_string\t$description\t"
...
elseif (!isset($_SESSION['server_num'])) {
  echo '   
  <p class="blockquote">
    Select a server from the available list below:<br>
  ...
  echo '
    " method="post">   
    <div class="col-sm">
      <select type="number" name="server_num" id="server_num" required="required" 
        oninvalid="this.setCustomValidity(\'You must select a server!\')" 
          class="btn btn-primary dropdown-toggle" style="width:200px" 
            onmousedown="if(this.options.length>8){this.size=8;}"  
              onchange="this.size=0;this.setCustomValidity(\'\');" 
                onblur="this.size=0;" >';
  ...
  while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    echo '<option value="' . $row['server_num'] . '">Server '
      . $row['server_num'] . '</option>';
  }
  ...
  <div class="col-sm"><br>
    <p>
      Enter a brief description of what\'s on the drive:<br>
      <small>
        Note: The description can only be 30 characters max.
      </small>
      <br><br>
      <textarea required="required" name="description" rows="2" cols="30"
        placeholder="Ubuntu Server 18.04.3 LTS">
      </textarea>
      <br>
    </p>
  </div>
