...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['student_id'])) {
  ...
  if ($drive_id && $student_id) {
    ...
    $student_update_query = "UPDATE students SET drive_id=$drive_id "
    ...
    if ($result_a && $result_b && $result_c) {
      ...
      success_string_gen('Your drive has been assigned.</h3><br><h5>
      ...
      $update .= "$drive_string\t$student_string\t" .
...
if (!isset($_SESSION['student_id'])) {
  ...
  echo '
    <p class="lead">
      Use the dropdown to select a hard drive for your studies here 
      at Delaware Tech.<br>
    </div>
    </p>
    <p>
    ...
    <div class="col-sm">
    <select name="drive_id" required="required" 
      oninvalid="this.setCustomValidity(\'Please select a drive number\')"
  ...
  // Fetch server list from database, then populate a dropdown 
  // list with the results
  $sql = "SELECT * FROM drives WHERE in_use=0 AND broken=0";
  $result = $db_conn->query($sql);
  while ($row = $result->fetch_assoc()) {
    echo '<option value="' . $row['drive_id'] . '">Drive '
      . $row['drive_num'] . '</option>';
  }
