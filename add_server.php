...
if (isset($_SESSION['admin_id']) && $_SERVER['REQUEST_METHOD'] != 'POST') {
  ...
  <form action="add_server.php" method="post">
    <table class="table" cellpadding="5">
      ...
      <th>Type</th>
      <tr>
        <td><input type="number" name="server_num" /></td>
        <td><input type="text" name="type" /> </td>
      </tr>
    </table>
  <br>
  <input class="btn-dark" type="submit" name="submit" value="Add server" />
    <br><br>
    <button class="btn-dark" type="button" name="back" 
      onclick="document.location.href=\'add_server.php\'">Go back</button>
  </form>';
}
...
elseif (isset($_SESSION['admin_id']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['server_num']) && is_numeric((int)$trimmed['server_num'])) {
    ...
    error_string_gen('Incorrect server number. Try again! Must be numeric.');
  }
